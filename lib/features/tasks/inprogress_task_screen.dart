// ignore_for_file: use_key_in_widget_constructors, avoid_print, use_build_context_synchronously, unused_import
...
class InProgressTasksScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text('In Progress Tasks', style: AppTextStyles.whiteBody(context)),
    ...
    try {
      ...
      if (orderId == null) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Cannot Complete: Invalid order ID')),
        );
        return;
      }
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Work completed successfully')),
      );
    } catch (e, stackTrace) {
...
void _sendCustomerNotification(String customerId, String orderId) {
  try {
    FirebaseFirestore.instance.collection('customer_notifications').add({
      'userId': customerId,
      'message': 'Work has completed on your order.',
      'orderId': orderId,
